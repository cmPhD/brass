function [w, V, invV, logdetV, an, bn, E_a, L] = ...
    bayes_linear_fit_ard(X, y, options)
    arguments
        X
        y
        options.MaxIterIsError (1,1) {mustBeNumericOrLogical} = false
    end
...
L_last = -realmax;
max_iter = Config.ArdMaxIter;
E_a = ones(D, 1) * c0 / d0;
for iter = 1:max_iter
    % covariance and weight of linear model
    invV = diag(E_a) + X_corr;
    V = inv(invV);
    % logdetV = - logdet(invV); <cm>
    logdetV = - logdetBL(invV);
    w = V * Xy_corr;
...
if options.MaxIterIsError && iter == max_iter
    error('Bayes:maxIter', ...
